...
-- Executes file from boot HDD during OS initialization (will be override in filesystem library later)
...
-- CHeck existence of specified path. It will be override after filesystem library initialization
...
-- Display title and currently required library when booting OS
...
local function UIRequire(module)
    ...
    local title, width, total = "EpycOS", 26, 14
    ...
    component.invoke(GPUAddress, "set", x + part, x + 2, string.rep("-", width - part))
...
filesystem.proxy(bootFilesystemProxy)

-- Replacing requireExists function after filesystem library
